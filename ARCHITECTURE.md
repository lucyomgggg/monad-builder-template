...
- **Stateless Runtime**: `monad.py` is only the entrypoint. Logic lives in `builder_runtime/`, while personality and operational targets are defined in `config.yaml`.
...
1.  **Orchestrator (`builder_runtime/app.py:main`)**:
...
2.  **Execution Round (`builder_runtime/app.py:run_once`)**:
...
3.  **Interaction Loop (`builder_runtime/llm.py:agent_turn`)**:
...
- **`builder_runtime/telos.py`**: Telos Core API client for semantic operations.
- **`builder_runtime/http_tools.py`**: Sanitized wrapper around `httpx` with follow-redirects, timeout management, and host allowlisting.
- **`builder_runtime/workspace.py`**: Safe file system operations restricted to the relative `workspace_dir`. Path traversal is strictly prevented.
- **`builder_runtime/tools.py`**: Tool schema plus dispatch layer that composes Telos / HTTP / workspace helpers.
- **`builder_runtime/llm.py`**: LLM loop and tool-call handling.
